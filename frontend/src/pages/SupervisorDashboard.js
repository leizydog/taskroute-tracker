...
import { useToast } from '../contexts/ToastContext'; // Keep using this context for consistency if needed, but UserModal uses internal state now
...
import {
  ...
  FiChevronDown, 
  FiAlertTriangle,
  FiTrash2 // Make sure this is imported
} from 'react-icons/fi';
import { Button, Card, StatValue, Input, Select, Alert, Badge } from '../components/atoms'; // Removed Avatar import if unused or use UserAvatar
import UserAvatar from '../components/atoms/UserAvatar'; // Assuming this exists based on AdminDashboard context
...
// --- Local Sub-Component: User List Item (From AdminDashboard) ---
  ...
  >
    ...
    <div className="flex-1 overflow-hidden">
      <div className="flex justify-between items-center">
          ...
          <Badge 
              text={employee?.role} 
              color={employee?.role === 'supervisor' ? 'purple' : 'blue'} 
              size="xs" 
          />
      </div>
...
// --- Local Sub-Component: Enhanced Confirmation Modal (From AdminDashboard) ---
const ConfirmationModal = ({ isOpen, title, message, type = 'danger', onConfirm, onClose, isLoading }) => {
  ...
  const typeConfig = {
    ...
    warning: {
      icon: FiInfo, // Changed to Info for warning or use FiArchive
      color: 'amber',
...
// --- IMPROVED USER MODAL (From AdminDashboard) ---
const UserModal = ({ mode = 'add', user = null, onClose, onSuccess }) => {
  const [formData, setFormData] = useState({
    ...
    email: '',
    password: '',
    role: 'user', // Supervisor can only create users mostly, but logic handles payload
    is_active: true,
    ...user
  });
    ...
    try {
      if (mode === 'add') {
        const payload = {
          ...
          role: 'user', // Supervisors force create 'user' role usually, adjust if they can create supervisors
          username: formData.username || formData.email.split('@')[0] 
        };
        
        // Supervisors typically only create standard users. 
        // If they can create other roles, add logic here.
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          {/* Supervisors usually can't change roles, so this is hidden or read-only */}
          {/* <div>
...
const SupervisorDashboard = () => {
  ...
  const navItems = [
    ...
    { id: 'user_management', label: 'Team Management', icon: <FiUsers /> },
  ...
  }, [isMapLoaded, mapLoadError]); // Removed toast from dependencies

  // 1. Helper: Add Temporary Popup Alert (Auto-dismiss)
  ...
  // 2. Helper: Add Persistent Notification (History + Popup)
  ...
  const handleNotificationClick = (notif) => {
    ...
    if (notif.context.tab === 'tasks') {
        toast.success('Navigated to Task #' + notif.context.itemId);
    } else if (notif.context.tab === 'user_management') {
        setSearchTerm(notif.context.itemName || '');
        toast.success('Navigated to User: ' + notif.context.itemName);
    }
  ...
  // ✅ Fix: Wrapped fetchData in useCallback
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // Supervisors see standard users and themselves
      const processedUsers = (Array.isArray(usersData) ? usersData : []).filter(u => u.role === 'user' || u.id === user.id).map(u => ({
          ...u,
  ...
  }, [user, logout]); // Removed toast from dependencies
    ...
    const connectWebSocket = () => {
        ...
        ws.onmessage = (event) => {
            try {
                ...
                // Supervisors care about their team members
                if (data.event === 'user_profile_updated' || data.event === 'user_avatar_updated') {
                    // Only refresh if it's a user we manage
                    api.getUsers().then(res => {
                       const allUsers = res.data || [];
                       const myUsers = allUsers.filter(u => u.role === 'user' || u.id === user.id);
  ...
  const handleUserSaved = (userData) => {
    fetchData();
    // Success view handled in UserModal
  };
  ...
  const handleExportTasksPDF = () => {
    ...
    doc.setFontSize(16);
    doc.text("TaskRoute - Team Task Report", 14, 13);

    doc.setTextColor(0, 0, 0);
    doc.setFontSize(10);
    doc.text(`Generated: ${timestamp}`, 14, 30);
    doc.text(`Supervisor: ${user?.full_name}`, 14, 35);
  ...
  const handleArchiveUser = (user) => {
    setConfirmModal({
        ...
        message: `Are you sure you want to deactivate ${user.full_name}? They will lose access to the system.`,
        isLoading: false,
        onConfirm: async () => {
            ...
            try {
                ...
                addAlert('info', 'User deactivated successfully.');
            } catch (err) {
  ...
  const stats = useMemo(() => {
    const totalTeamMembers = employees.filter(e => e.role === 'user').length;
    ...
    return { totalTeamMembers, activeTasks, completedToday };
  }, [employees, tasks]);
      ...
      <aside className={`fixed z-30 inset-y-0 left-0 transform ${mobileOpen ? 'translate-x-0 shadow-xl' : '-translate-x-full'} md:translate-x-0 md:static transition-transform duration-300 ease-in-out w-64 bg-slate-900 text-slate-100 p-4 flex flex-col border-r border-slate-800`}>
        <div className="flex items-center justify-between mb-8 px-2">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-bold text-white tracking-tight">Supervisor</h1>
          </div>
        ...
        <nav className="flex-1 space-y-1.5 overflow-y-auto px-2">
          {navItems.map((item) => (
            ...
            >
              <span className={`${activeTab === item.id ? 'text-white' : 'text-slate-500 group-hover:text-purple-400'}`}>{item.icon}</span>
        ...
        <div className="mt-auto px-2 pt-4 border-t border-slate-800/50">
            <div className="bg-slate-800/50 rounded-lg p-3 border border-slate-700">
                <div className="flex items-center gap-3">
                    <div className="w-8 h-8 flex-shrink-0 rounded-full bg-purple-900 flex items-center justify-center font-bold text-purple-300 overflow-hidden border border-slate-200 dark:border-slate-700">
                      {user?.avatar_url ? (
                        <img 
                    ...
                    <div className="overflow-hidden">
                        ...
                        <p className="text-xs text-purple-400 font-mono uppercase tracking-wider">Supervisor</p>
                    </div>
                </div>
            </div>
        </div>
          ...
          <div className="flex items-center gap-4">
                                    ...
                                    notifications.map((notif) => (
                                        <div 
                                            ...
                                            className={`px-4 py-3 border-b border-slate-100 dark:border-slate-700 last:border-0 transition-colors 
                                                ...
                                                ${notif.context ? 'cursor-pointer hover:bg-purple-50 dark:hover:bg-purple-900/20' : ''}`}
                                        >
            ...
            <div className="relative">
                <button 
                  ...
                  className={`flex items-center gap-2 p-1 pl-3 pr-2 rounded-full transition-colors border ${showUserMenu ? 'bg-slate-100 dark:bg-slate-800 border-purple-400 dark:border-purple-600' : 'border-transparent hover:bg-slate-100 dark:hover:bg-slate-800'}`}>
                  <div className="w-8 h-8 flex-shrink-0 rounded-full bg-purple-100 dark:bg-purple-900 flex items-center justify-center font-bold text-purple-600 dark:text-purple-300 overflow-hidden border border-slate-200 dark:border-slate-700">
                    {user?.avatar_url ? (
                      <img 
                        src={user.avatar_url.startsWith('http') ? user.avatar_url : `${API_URL}${user.avatar_url}`}
                        alt={user?.full_name || 'User'} 
            ...
            {activeTab === 'overview' && (
              <div className="space-y-6">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  <StatValue label="Total Team Members" value={stats.totalTeamMembers} icon={<FiUsers className="text-purple-500" />} />
                  ...
                  <div className="space-y-6">
                      <Card>
                        ...
                        <div className="space-y-3">
                          <Button variant="primary" fullWidth onClick={() => setIsCreateTaskModalOpen(true)} icon={FiPlus}>New Task</Button>
                          <Button variant="secondary" fullWidth onClick={() => { setActiveTab('user_management'); setTimeout(() => setUserModalConfig({ isOpen: true, mode: 'add', user: null }), 100); }} icon={FiUserPlus}>Add Team Member</Button>
                        </div>
                      </Card>
                  </div>
                </div>
                ...
                <div className="lg:col-span-1 flex flex-col h-full">
                  <Card className="flex-1 flex flex-col p-0 overflow-hidden border-0 shadow-none bg-transparent">
                    <div className="bg-white dark:bg-slate-800 p-4 rounded-t-xl border border-slate-200 dark:border-slate-700 shadow-sm z-10">
                        <h3 className="font-semibold text-slate-800 dark:text-slate-100 mb-3">Team Selection</h3>
                    ...
                    <div className="flex-1 overflow-y-auto space-y-2 p-2 bg-slate-100 dark:bg-slate-900/50 border-x border-b border-slate-200 dark:border-slate-700 rounded-b-xl">
                      {filteredEmployees.length > 0 ? (
                        ...
                        <div className="text-center py-8 text-slate-500">No matching team members found</div>
                      )}
                    </div>
                  </Card>
                </div>
                <div className="lg:col-span-2 h-full overflow-hidden flex flex-col">
                  {selectedEmployee ? (
                         ...
                         <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700 mb-6 shadow-sm">
                            <div className="flex items-start justify-between">
                                <div className="flex gap-4">
                                    <div className="w-24 h-24 flex-shrink-0 rounded-full bg-purple-100 dark:bg-purple-900 flex items-center justify-center font-bold text-purple-600 dark:text-purple-300 overflow-hidden border border-slate-200 dark:border-slate-700">
                     ...
                     <div className="h-full flex flex-col items-center justify-center text-slate-400 bg-slate-100 dark:bg-slate-800/30 rounded-xl border-2 border-dashed border-slate-300 dark:border-slate-700">
                        ...
                        <p className="text-lg font-medium">Select a team member to view KPI metrics</p>
                     </div>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-6">
                 <div className="flex justify-between items-center">
                     ...
                     <div className="flex gap-3">
                        <Button 
                            variant="primary" 
                            onClick={() => setUserModalConfig({ isOpen: true, mode: 'add', user: null })} 
                            icon={FiUserPlus}
                        >
                            Add Team Member
                        </Button>
                 ...
                 <Card className="overflow-hidden p-0">
                     <div className="p-4 border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50">
                         <div className="relative">
                            ...
                            <input 
                                className="w-full pl-10 pr-4 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 focus:ring-2 focus:ring-purple-500 outline-none transition-all"
                ...
                <div className="bg-slate-900 rounded-xl p-8 text-white shadow-lg relative overflow-hidden">
                    <div className="relative z-10">
                        ...
                        <p className="text-slate-300 max-w-2xl">
                            Analyze your team's performance patterns and efficiency metrics.
                        </p>
                    </div>
                    <div className="absolute right-0 top-0 w-64 h-64 bg-purple-500 rounded-full blur-3xl opacity-20 transform translate-x-1/3 -translate-y-1/3"></div>
                </div>
                
                <Card className="p-6">
                  <div className="mb-6 border-b border-slate-200 dark:border-slate-700 pb-4">
                      <h3 className="text-lg font-bold text-slate-900 dark:text-slate-100 flex items-center gap-2">
                        <FiTrendingUp className="text-purple-500" /> Feature Importance Analysis
                      </h3>
                      <p className="text-sm text-slate-500 mt-1">
                          Understand what drives your team's efficiency.
                      </p>
                  </div>
