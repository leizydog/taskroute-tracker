...
const UserListItem = ({ employee, isSelected, onClick, stats }) => (
  <motion.div
    whileHover={{ x: 3 }}
    onClick={onClick}
    className={`p-3 rounded-lg border cursor-pointer transition-all duration-150 flex items-center gap-3 ${isSelected
    ...
    <div className="flex-1 overflow-hidden">
      ...
      {stats && (
        <div className="flex items-center gap-2 mt-1.5">
          <span className="text-[10px] font-medium text-slate-600 dark:text-slate-400 bg-slate-100 dark:bg-slate-700 px-1.5 py-0.5 rounded">
            Tasks: {stats.total}
          </span>
        ...
        <nav className="flex-1 space-y-1.5 overflow-y-auto px-2">
          {navItems.map((item) => (
            <button
              ...
              className={`w-full flex items-center gap-3 px-3 py-3 rounded-lg text-sm transition-all duration-200 group ${activeTab === item.id
